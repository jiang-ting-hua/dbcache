...
//从配置文件中,取出数据,保存于结构体.
func ParseConfInterface(fileName string, result interface{}) (err error) {
	...
	//fmt.Printf("%v %v %v",tElem.Name(),tElem.String(),tElem.NumMethod())
	//os.Exit(1)
	//result必须是一个指针
	...
	//result必须是一个接口,并且结构名与配置文件中分段[]名要一样.
	if tElem.Kind() != reflect.Interface {
		fmt.Println("必须是一个接口")
	}
	vElem := v.Elem().Elem()
	//tt,ok := result.(interface{})
	//fmt.Println(ok,tt)
	// uu, ok := (tt).Users
	//fmt.Println(ok,uu)
	//os.Exit(1)
	for i := 0; i < vElem.NumField(); i++ {
		...
		//tag := vfield.Tag.Get("conf") //到得该字段的Tag
		//fmt.Printf("tag=%v \n",tag)
		fmt.Printf("%v \n",vfield.Type())
